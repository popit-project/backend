...
public class StoreSellerController {
    ...
    public ResponseEntity<?> sellerEntered(@AuthenticationPrincipal String userId,
        ...
        @RequestPart("sellerDTO") StoreSellerDTO sellerDTO) throws IOException {
    ...
    @GetMapping("/store/{storeName}/storeHome")
    public ResponseEntity<?> sellerStoreInfo(@PathVariable("storeName") String storeName) {
        StoreEntity store = storeRepository.findByStoreName(storeName).orElseThrow(
